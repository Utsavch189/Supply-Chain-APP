...
function RetailerReceiveHist({navigation}) {
  const route=useRoute();
    const[head,setHead]=useState(null);
    const[data,setData]=useState(null);

    useEffect(()=>{
      if(route.params.token){myaxios(JSON.parse(route.params.token)).get(`${url}/retailer/receive_stock_history`)
      .then(res=>{
    ...
    if(!data && !head){
        return(
          <>
            ...
            <HistoryNav navigation={navigation} state='Received' token={route.params.token} type='retailer'/>
            <Loader/>
          </>
        )
      }
...
export default RetailerReceiveHist