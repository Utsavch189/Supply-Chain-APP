...
function DistributeStockHist({navigation}) {
  ...
  const route=useRoute();
    ...
    useEffect(()=>{
      if(route.params.token){myaxios(JSON.parse(route.params.token)).get(`${url}/distributor/get_dayBYdayDistribute`)
      .then(res=>{
    ...
    if(!data && !head && !tittle){
        return(
          <>
            ...
            <HistoryNav navigation={navigation} state='Distributed' token={route.params.token} type='distributor'/>
            <Loader/>
          </>
        )
      }
...
export default DistributeStockHist